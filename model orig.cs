...
namespace AnimalChange
{
    class model
    {
        ...
        //if args[5] is 1, the errormessages will be returned as a string instead of being written to the error-file
        //if args[6] is present, it must contain the full path to the start xml file (often called system.xml) e.g. c:\Project\inputfiles\mysystem.xml
        ...
        public void run(string[] args)
        {
            ...
            {
                system[0] = args[6];
                string[] dumstring= system[0].Split('.');
                system[1] = dumstring[0] + "Alternative.xml";
            ...
            for (int settingsID = minSetting; settingsID <= maxSetting; settingsID++)
            {
                ...
                if (args.Length != 5 && args.Length != 0 && args.Length != 6 && args.Length != 7)
                {
...
//                        if (args[5].CompareTo("1") == 0)
                ...
                if (settings.doesIDExist(settingsID))
                {
                    ...
                    if (args.Length != 0 && args[0].CompareTo("-1") != 0)
                    {
                        int lastBackslash = farmName.LastIndexOf('\\');
                        string farmFileName = "";
                        string pathName = "";
                        if (lastBackslash > 0)
                        {
                            farmFileName = farmName.Substring(lastBackslash + 1);
                            pathName = farmName.Substring(0,lastBackslash+1);
                        }
                        string[] names = farmFileName.Split('.');

                        farmName = pathName + "_" + args[0] + "_" + args[1] + "." + names[names.Count() - 1];
