...
//        fCO2 = C_ToolToCopy.fCO2;
    ...
    */
    ...
    /*!
     \param JulianDay as an integer argument,
     \param layerNo an integer argument (1 or 2)
     \param temperature temperature of the layer (Celsius) as a double argument
     \param droughtCoefficient drought coefficient as a double argument
     \param FOMtransportIn input of FOM from previous layer as a double argument
     \param FOMtransportOut output of FOM from the layer
     \param FOMCO2 loss of FOM as CO2
     \param HUMtransportIn input of HUM from previous layer as a double argument
     \param HUMtransportOut  output of HUM from the layer
     \param HUMCO2 loss of HUM as CO2
     \param BiochartransportIn input of biochar from previous layer as a double argument
     \param BiochartransportOut output of biochar from the layer
     \param BiocharCO2 loss of biochar as CO2
     \param newHUM new HUM resulting from decomposition of FOM
     \param newROM new ROM resulting from decomposition of HUM
     */
        ...
        ref double newHUM, ref double newROM)
    {
        ...
        double initialFOM = fomc[layerNo];
        double FomAfterDecom = rk4decay(fomc[layerNo], timeStep, FOMdecompositionrate, temporaryCoefficient);
        double FOMdecomposition = fomc[layerNo] - FomAfterDecom;
        double SpikeFomAfterDecom = rk4decay(spike_fomc[layerNo], timeStep, FOMdecompositionrate, temporaryCoefficient);
        
        newHUM = FOMdecomposition * humification;
        double newSpikeHUM = (spike_fomc[layerNo]-SpikeFomAfterDecom) * humification;
        FOMCO2 = FOMdecomposition * (1 - humification);
        CO2+=FOMCO2;
        double SpikeFOMtransportOut = 0;
        if (layerNo == 0)
        {
            FOMtransportOut = FomAfterDecom * tF;
            SpikeFOMtransportOut = SpikeFomAfterDecom * tF;
        }
        else
            FOMtransportOut = 0.0;
        fomc[layerNo] = FomAfterDecom + FOMtransportIn - FOMtransportOut;
        double test = (initialFOM + FOMtransportIn) - (fomc[layerNo] + FOMtransportOut + newHUM + FOMCO2);

        //do HUM
        if (zeroRatesForDebugging)
            HUMdecompositionrate = 0;
        double InitialHUM = humc[layerNo];
        double HumAfterDecom = rk4decay(humc[layerNo], timeStep, HUMdecompositionrate, temporaryCoefficient);
       
        double HUMdecomposition = humc[layerNo] - HumAfterDecom;
        ...
        if (layerNo == 0)
        {
            humc[layerNo] = HumAfterDecom + newHUM + HUMtransportIn;  //Note; HUMtransportOut is calculated from the decomposed FOM, so is accounted for in the value of HumAfterDecom
            test2 = (InitialHUM + HUMtransportIn + newHUM) - (humc[layerNo] + newROM + HUMCO2 + HUMtransportOut);
        }
        else
        {
            humc[layerNo] = HumAfterDecom + newHUM + HUMtransportIn + HUMtransportOut; //HUMtransportOut is recycled in the lower layer
            test2 = (InitialHUM + HUMtransportIn + newHUM) - (humc[layerNo] + newROM + HUMCO2);
        }

        //do ROM
        if (zeroRatesForDebugging)
            ROMdecompositionrate = 0;
        double InitialROM = romc[layerNo];
        double RomAfterDecom = rk4decay(romc[layerNo], timeStep, ROMdecompositionrate, temporaryCoefficient);
        double ROMdecomposition = romc[layerNo] - RomAfterDecom;
        ROMCO2 = fCO2 * ROMdecomposition;        
        ROMtransportOut = ROMdecomposition * (1 - fCO2);
        double test3 = 0.0;
        if (layerNo == 0)
        {
            romc[layerNo] = RomAfterDecom + newROM + ROMtransportIn; //Note; ROMtransportOut is calculated from the decomposed HUM, so is accounted for in the value of RomAfterDecom
            test3 = (InitialROM + ROMtransportIn + newROM) - (romc[layerNo] + ROMCO2 + ROMtransportOut);
        }
        else
        {
            romc[layerNo] = RomAfterDecom + newROM + ROMtransportIn + ROMtransportOut;
            test3 = (InitialROM + ROMtransportIn + newROM) - (romc[layerNo] + ROMCO2);
        }

        //use ROM decomposition rate for biochar
        double InitialBiochar = biocharc[layerNo];
        double BiocharAfterDecom = rk4decay(biocharc[layerNo], timeStep, ROMdecompositionrate, temporaryCoefficient);

        double Biochardecomposition = biocharc[layerNo] - BiocharAfterDecom;
        BiocharCO2 = fCO2 * Biochardecomposition;
        BiochartransportOut = Biochardecomposition * (1 - fCO2);
        double test4 = 0.0;
        if (layerNo == 0)
        {
            biocharc[layerNo] = BiocharAfterDecom + BiochartransportIn; //Note; BiochartransportOut is calculated from the decomposed Biochar, so is accounted for in the value of BiocharAfterDecom
            test4 = (InitialBiochar + BiochartransportIn) - (biocharc[layerNo] + BiocharCO2 + BiochartransportOut);
        }
        else
        {
            biocharc[layerNo] = BiocharAfterDecom + BiochartransportIn + BiochartransportOut; //Note; BiochartransportOut is calculated from the decomposed Biochar, so is accounted for in the value of BiocharAfterDecom
            test4 = (InitialBiochar + BiochartransportIn) - (biocharc[layerNo] + BiocharCO2);
        }
    ...
    */
    ...
    //! Calculate func function in the rk4decay integration function.
    /*!
      \param amount the amount as a double argument
      \param coeff the coefficient as a double argument
    ...
    */
    ...
    //! A public member, Calculate CtoNFactor. Taking one argument and
    /*!
